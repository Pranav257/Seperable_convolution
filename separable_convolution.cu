                   ...
                   int num_rows, int half_width)
{
    ...
    float *kernel_d = NULL;
    cudaMalloc((void**)&kernel_d, (2 * half_width + 1) * sizeof(float));
    cudaMemcpy(kernel_d, kernel, (2 * half_width + 1) * sizeof(float), cudaMemcpyHostToDevice);
	...
	convolve_rows_kernel_naive<<<grid, thread_block>>>(matrix_temp_d,  matrix_d, kernel_d,\
                                                         num_cols,  num_rows,  half_width); 
    cudaDeviceSynchronize();
    
	convolve_columns_kernel_naive<<<grid, thread_block>>>(gpu_result_d, matrix_temp_d,  kernel_d,\
                                                             num_cols,  num_rows,  half_width); 
    cudaDeviceSynchronize();
